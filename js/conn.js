// ============================================================
// cTrader Bot Studio Pro — High-Performance WebSocket Engine
// ============================================================
  ...
  const c = {
    ...
    get creds() {
      return this.config
        ...
        : { env: 'demo', clientId: '', clientSecret: '', accessToken: '', accountId: '' };
    },
    ...
    connect() {
      if (this.state === 'connecting' || this.state === 'connected') {
        this.log('WS engine already active (' + this.state + ').', 'warn');
        return;
      }
      const creds = this.creds;
      if (!creds.clientId || !creds.clientSecret) {
        this.log('Client ID or Client Secret missing. Connection paused.', 'err');
      ...
      this.log('Opening secure socket connection to ' + this.host() + '…', 'info');

      try { if (this.ws) this.ws.close(); } catch (e) { /* silent close */ }
      ...
      ws.onopen = () => {
        ...
        this.log('Socket link established — performing App Handshake…', 'info');
        this.send({
          clientMsgId: this.nextId(),
          payloadType: P.APPLICATION_AUTH_REQ,
          payload: { clientId: creds.clientId, clientSecret: creds.clientSecret }
        });
        this.phase = 'app_auth';
      };
      ...
      ws.onmessage = (e) => {
        ...
        if (typeof e.data === 'string') {
          this.handleRaw(e.data);
        } else if (e.data instanceof Blob) {
          e.data.text().then((t) => this.handleRaw(t)).catch(() => {});
        }
      };

      ws.onerror = () => {
        this.log('WebSocket transport handshake failed (host: ' + this.host() + ':5036).', 'err');
      };
      ...
      ws.onclose = (ev) => {
        ...
        if (this.intentional) {
          ...
          this.log('Websocket closed gracefully.', 'warn');
          if (this.onDisconnected) this.onDisconnected('manual');
          return;
        }
        if (!this._everOpened) {
          this.log('Remote host unreachable. Verify network status and credentials.', 'err');
          this.status('Offline', 'off');
          return;
        }
        this.log('Link terminated (' + ev.code + '). Reconnecting…', 'warn');
    ...
    disconnect() {
      ...
      this.phase = '';
      this.status('Disconnected', 'off');
      this.log('Manual disconnection triggered.', 'warn');
    },
    ...
    connectFresh() {
      ...
      if (this.ws) { try { this.ws.onclose = null; this.ws.close(); } catch (e) { /* noop */ } this.ws = null; }
      this.state = 'idle';
      this.log('Flushing credentials — launching fresh socket…', 'info');
      this.connect();
    },
    ...
    handleRaw(data) {
      ...
      switch (type) {
        case P.APPLICATION_AUTH_RES: {
          ...
          this.log('App authenticated successfully.', 'ok');
          ...
          if (creds.accessToken && creds.accountId) {
            ...
            this.log('No access token — click "Authorize with cTrader" to authenticate.', 'warn');
          }
          break;
        }
        case P.GET_ACCOUNTS_BY_ACCESS_TOKEN_RES: {
          const accounts = payload.ctidTraderAccount || [];
          this.log('Discovered ' + accounts.length + ' accounts linked to token.', 'ok');
        ...
        case P.CLIENT_DISCONNECT_EVENT:
          this.log('Proxy session terminated by host (' + (payload.reason || '') + ').', 'warn');
          this.status('Reconnecting…', 'mid');
          this.scheduleReconnect();
          if (this.ws) { try { this.ws.onclose = null; this.ws.close(); } catch (e) { /* noop */ } this.ws = null; }
          this.state = 'idle';
        ...
        case P.ERROR_RES: {
          ...
          if (TOKEN_ERRORS.some((t) => code.toUpperCase().indexOf(t) !== -1)) {
            this.log('OAuth access token expired/invalidated (' + code + ').', 'err');
            if (!this.tokenErrorShown) {
            ...
            this.log('Proxy Error Response [' + code + ']: ' + (payload.description || ''), 'err');
          }
          break;
        }
        case P.ORDER_ERROR_EVENT:
          this.log('Broker order rejected [' + (payload.errorCode || '') + ']: ' + (payload.description || ''), 'err');
          break;
        default:
    ...
    authFailed(payload) {
      ...
      this.teardownTimers();
      this.log('Verify Client ID and Secret, then retry.', 'warn');
      this.intentional = true; // prevent hot-loop
      this.state = 'idle';
    ...
    accountAuth(accountId, token) {
      ...
      this.log('Authenticating Broker Account #' + id + '…', 'info');
      this.phase = 'account_auth';
      this.send({
        clientMsgId: this.nextId(),
        payloadType: P.ACCOUNT_AUTH_REQ,
        payload: { ctidTraderAccountId: id, accessToken: token }
      });
    },
    ...
    requestAccounts() {
      ...
      if (!token) return;
      this.log('Querying linked accounts for token…', 'info');
      this.send({
        clientMsgId: this.nextId(),
        payloadType: P.GET_ACCOUNTS_BY_ACCESS_TOKEN_REQ,
    ...
    startPing() {
      ...
      this.pingTimer = setInterval(() => {
        ...
        if (id) {
          this.send({
            clientMsgId: this.nextId(),
            payloadType: P.TRADER_REQ,
            payload: { ctidTraderAccountId: parseInt(id, 10) }
          });
        }
      }, 30000);
    },
    ...
    startWatchdog() {
      ...
      this.watchdogTimer = setInterval(() => {
        ...
        if (this.state === 'connected' && idle > 75000) {
          this.log('Channel silent for ' + Math.round(idle / 1000) + 's — resetting socket…', 'warn');
          this.forceReconnect();
        } else if (this.state === 'connecting' && Date.now() - this.connectStartedAt > 20000) {
          this.log('App Handshake stalled — restarting socket…', 'warn');
          this.forceReconnect();
        }
      }, 5000);
    },
    ...
    forceReconnect() {
      this.state = 'idle';
      if (this.ws) { try { this.ws.onclose = null; this.ws.close(); } catch (e) { /* silent close */ } this.ws = null; }
      this.scheduleReconnect();
    },
    ...
    scheduleReconnect() {
      ...
      if (this.reconnectAttempts > 8) {
        this.giveUp = true;
        this.state = 'idle';
        this.status('Failed', 'off');
        this.log('Maximum retry attempts reached (8). Please reconnect manually.', 'err');
        return;
      }
      const base = Math.min(1000 * Math.pow(2, this.reconnectAttempts), 30000);
      const jitter = Math.floor(Math.random() * 2000);
      const delay = base + jitter;
      this.log('Re-establishing socket link in ' + Math.round(delay / 1000) + 's (attempt ' + this.reconnectAttempts + ')…', 'warn');
